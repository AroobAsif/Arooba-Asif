...
void listItems() {
    if (itemCount == 0) {
        ...
        for (int i = 0; i < itemCount; i++) {
            cout << GREEN << i + 1 << ". " << items[i] << " - " << itemPrices[i] << " Pkr" << RESET << endl;
        }
    }
}
...
void addItem() {
    ...
    while (!(cin >> itemPrices[itemCount])) {
...
void updateItem() {
    ...
    cin.ignore();
    getline(cin, items[itemNumber - 1]);

    cout << YELLOW << "Enter new item price: " << RESET;
    while (!(cin >> itemPrices[itemNumber - 1])) {
        cout << RED << "Invalid input. Please enter a valid price: " << RESET;
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    }
...
void buyItem() {
    ...
    for (int i = 0; i < quantity; i++) {
        cart[cartItemCount] = itemNumber - 1;
        cartItemCount++;
    }
